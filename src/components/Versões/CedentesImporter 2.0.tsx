...
export default function CedentesImporter() {
  ...
  async function loadFromServer() {
    try {
      ...
      const { listaCedentes: saved, meta } = json.data;
      if (Array.isArray(saved) && saved.length) {
        setListaCedentes(saved);
        setDedupedNames(saved.map((c: any) => c.nome_completo));
        // Se quiser restaurar meta também:
        // setNamesSheet(meta?.namesSheet ?? namesSheet);
        // setPrograms((prev)=> prev.map(p => {
        //   const m = meta?.programs?.find((x:any)=>x.key===p.key);
        //   return m ? {...p, sheet:m.sheet, colName:m.colName, colPoints:m.colPoints} : p;
        // }));
